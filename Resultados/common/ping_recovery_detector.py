...
def check_ping_detection(path,ip):
   df = pd.read_csv(path, header=0, na_filter=True, float_precision="round_trip", quotechar='"', sep=",",
                       quoting=csv.QUOTE_NONNUMERIC)
   dfVm = df[df["hostname"]==ip]
   return (dfVm.tail(1)["ret_code"] == 0.0).bool()
...
if __name__ == "__main__":
   if len(sys.argv) == 2:
      ...
      for ping_path in df[merge_key]:
         dict_list.append({
               merge_key : ping_path,   
               'vm%s_recovered_ping' % vm_id: check_ping_detection(os.path.join(data_path, ping_path),ip)
            })
