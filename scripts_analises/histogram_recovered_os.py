...
for i, file_name in enumerate(sorted_files):
    ...
    df = df.dropna(subset=['count_recovered_vms_ssh'])
    unique_values_solr = df['count_recovered_vms_ssh'].unique()
    unique_values_solr.sort()

    total_rows = len(df)

    
    percentages_os =  [len(df[df['count_recovered_vms_ssh']==value]) / total_rows * 100 for value in unique_values_solr]
 
    ax.hist(df['count_recovered_vms_ssh'], bins=unique_values_solr, cumulative=-1, density=True,
...
ax.set_xlabel('Count of Recovered VMs (OS)')
ax.set_ylabel('Percentage [0:1]')
ax.legend()
plt.savefig('cumulative_hist_ssh.pdf', bbox_inches='tight', pad_inches=0)
