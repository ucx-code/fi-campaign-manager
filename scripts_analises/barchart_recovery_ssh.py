...
for i, file_name in enumerate(os.listdir(folder_path)):
    if file_name.endswith('.csv'):
        ...
        total_rows = len(df)
        count_probabilities = df['count_recovered_vms_ssh'].value_counts() / total_rows * 100
...
plt.xlabel('Count of Recovered VMs (OS)',fontsize=12)
...
plt.savefig('recovery_barchart_ssh.pdf', bbox_inches='tight', pad_inches=0)
